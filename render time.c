...
int main() {
    ...
    do {
        printf("\nEnter the following details to calculate the render time of the video:\n");
        ...
        if (resolution == 2160 && ram == 32) {
            ...
            base_render_seconds = base_render_seconds * 3.0;
            est_render_time = base_render_seconds / 60.0;
            min = est_render_time * 0.8;
            printf("The estimated render time would be around %.2f to %.2f mins.\n", min, est_render_time);
        }
        ...
        printf("\nDo you want to calculate another render time? (y/n): ");
        scanf(" %c", &choice);

    } while (choice == 'y' || choice == 'Y');

    printf("\nThanks for using Video Render Estimator!\n");
